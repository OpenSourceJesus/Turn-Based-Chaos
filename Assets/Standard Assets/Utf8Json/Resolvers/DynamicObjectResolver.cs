...
namespace Utf8Json.Resolvers.Internal
{
    ...
    internal static class DynamicObjectTypeBuilder
    {
        ...
        {
            var ti = typeof(T)._GetTypeInfo();

            if (ti.IsNullable())
            {
                ti = ti.GenericTypeArguments[0]._GetTypeInfo();
            ...
            Type elementType;
            if (typeof(Exception)._GetTypeInfo().IsAssignableFrom(ti))
            {
        ...
        static TypeInfo BuildType(DynamicAssembly assembly, Type type, Func<string, string> nameMutator, bool excludeNull)
        {
            ...
            return typeBuilder._CreateTypeInfo();
        }
        ...
        static void BuildSerialize(Type type, MetaType info, ILGenerator il, Action emitStringByteKeys, Func<int, MetaMember, bool> tryEmitLoadCustomFormatter, bool excludeNull, bool hasShouldSerialize, int firstArgIndex)
        {
            ...
            var typeInfo = type._GetTypeInfo();
            ...
            foreach (var item in info.Members.Where(x => x.IsReadable))
            {
                if (excludeNull || hasShouldSerialize)
                {
                    ...
                    if (excludeNull)
                    {
                        if (item.Type._GetTypeInfo().IsNullable())
                        {
        ...
        static void EmitSerializeValue(TypeInfo type, MetaMember member, ILGenerator il, int index, Func<int, MetaMember, bool> tryEmitLoadCustomFormatter, ArgumentField writer, ArgumentField argValue, ArgumentField argResolver)
        {
            ...
            else if (jsonPrimitiveTypes.Contains(t))
            {
                ...
                argValue.EmitLoad();
                member.EmitLoadValue(il);
                il.EmitCall(typeof(JsonWriter)._GetTypeInfo().GetDeclaredMethods("Write" + t.Name).OrderByDescending(x => x.GetParameters().Length).First());
            }
            else
            {
                argResolver.EmitLoad();
        ...
        static void BuildDeserialize(Type type, MetaType info, ILGenerator il, Func<int, MetaMember, bool> tryEmitLoadCustomFormatter, bool useGetUninitializedObject, int firstArgIndex)
        {
            ...
            // read member loop
            {
                ...
                il.EmitCall(typeof(ArraySegment<byte>)._GetRuntimeProperty("Offset").GetGetMethod());
                il.Emit(OpCodes.Add);
                il.EmitStloc(p);

                // rest = arraySegment.Count
                il.EmitLdloca(keyArraySegment);
                il.EmitCall(typeof(ArraySegment<byte>)._GetRuntimeProperty("Count").GetGetMethod());
                il.EmitStloc(rest);
        ...
        static void EmitDeserializeValue(ILGenerator il, DeserializeInfo info, int index, Func<int, MetaMember, bool> tryEmitLoadCustomFormatter, ArgumentField reader, ArgumentField argResolver)
        {
            ...
            {
                reader.EmitLoad();
                il.EmitCall(typeof(JsonReader)._GetTypeInfo().GetDeclaredMethods("Read" + t.Name).OrderByDescending(x => x.GetParameters().Length).First());
            }
            else
            {
                argResolver.EmitLoad();
        ...
        {
            public static readonly ConstructorInfo ObjectCtor = typeof(object)._GetTypeInfo().DeclaredConstructors.First(x => x.GetParameters().Length == 0);
...
#endif
            public static readonly ConstructorInfo InvalidOperationExceptionConstructor = typeof(System.InvalidOperationException)._GetTypeInfo().DeclaredConstructors.First(x => { var p = x.GetParameters(); return p.Length == 1 && p[0].ParameterType == typeof(string); });
