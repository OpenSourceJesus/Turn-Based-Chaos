    ...
    public sealed class AttributeFormatterResolver : IJsonFormatterResolver
    {
        ...
        static class FormatterCache<T>
        {
            ...
            static FormatterCache()
            {
...
#else
                var attr = typeof(T)._GetTypeInfo().GetCustomAttribute<JsonFormatterAttribute>();
#endif
                if (attr == null)
                {
                    return;
                ...
                try
                {
                    if (attr.FormatterType.IsGenericType && !attr.FormatterType._GetTypeInfo().IsConstructedGenericType())
                    {
