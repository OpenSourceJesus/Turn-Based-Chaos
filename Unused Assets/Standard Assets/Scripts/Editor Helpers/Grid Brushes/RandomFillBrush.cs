...
namespace GridGame
{
	...
	public class RandomFillBrush : GridBrush
	{
		...
		public virtual bool PointIsOpen (Vector2Int point)
		{
			...
			// 	return Physics2D.OverlapPoint(GameManager.GetSingleton<World>().tilemaps[0].GetCellCenterWorld(point.ToVec3Int()), whatBlocksMe) == null;
			// else
			// {
			// 	foreach (ObjectInWorld worldObject in GameManager.GetSingleton<World>().worldObjects)
			// 	{
			// 		if (worldObject.trs.GetUnrotatedRect().Contains(point) && whatBlocksMe.MaskContainsLayer(worldObject.go.layer))
			// 			return false;
			// 	}
			// 	return !GameManager.GetSingleton<World>().tilemaps[0].HasTile(point.ToVec3Int());
			// }
		}
